...
public final class TrackTagsTableCellFactory implements TagsTableCellFactory {

    private static final Logger LOG = LoggerFactory.getLogger(TrackTagsTableCellFactory.class);
    ...
    public TableCell<LibraryItemRow, String> call(TableColumn<LibraryItemRow, String> param) {
        ...
        return new TextFieldTableCell<>() {
            ...
            private final HBox tagsHBox = new HBox();

            {
                tagsHBox.setSpacing(5);
                tagsHBox.setAlignment(Pos.CENTER_LEFT);
                tagsHBox.setOnMouseEntered(_ -> {
                    var track = (Track) getTableRow().getItem().getSource();
                    if (Reaction.THUMB_UP != track.reaction()) {
                        tagsHBox.getChildren().add(createLikeBtn(FontIcon.of(FontAwesomeRegular.HEART)));
                    }
                });
                tagsHBox.setOnMouseExited(_ -> {
                    var track = (Track) getTableRow().getItem().getSource();
                    if (Reaction.THUMB_UP != track.reaction()) {
                        tagsHBox.getChildren().removeLast();
                    }
                });
            }
                ...
                if (itemRow instanceof TrackItemRow trackItemRow) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        if (!inPlaylists.isEmpty()) {
                            tagsHBox.getChildren().add(FontIcon.of(FontAwesomeSolid.HEADPHONES_ALT));
                        }
                        ...
                        if (!inCollections.isEmpty()) {
                            tagsHBox.getChildren().add(FontIcon.of(FontAwesomeSolid.CLONE));
                        }
                        setGraphic(tagsHBox);
                    });
                }
